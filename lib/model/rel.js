...
function set2One (value, options) {
  if (_.isPlainObject(value)) {
    if (!_.isNil(value.id)) {
      assert(value.id > 0, this.name + ': bad value')
      return value.id
    } else {
      if (options.rec_class) {
        // TODO: Properly manage the "other" cases (updates...)
        assert(options.rec_class, 'missing rec in options')
        var newRec = new options.rec_class(options.rec_session, options.rec_model)
        newRec.set(_.pickBy(value, (v, k) => k.indexOf('.') < 0))
        return newRec
      }
  ...
  if (options.oc && _.isPlainObject(value)) {
    ...
    if (value.add) {
      _.each(value.add, (toAdd) => {
        res.splice(toAdd[0], 0, set2One.call(this, toAdd[1], options))
      })
    }
    if (value.remove) {
      _.remove(res, (x) => {
        return value.remove.indexOf(x.id) > -1
      })
    }
    if (value.update) {
    ...
    try {
      res.push(set2One.call(this, value, options))
    } catch (err) {
      if (checkRM(value)) {
  ...
  if (options.rpc) {
    ...
    _.each(value, (element) => {
      if (_.isPlainObject(element)) {
        if (element.id < 0) {
          // TODO: Properly manage the "other" cases (updates...)
          assert(options.rec, 'missing rec in options')
