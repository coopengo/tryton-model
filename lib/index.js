...
function alterDomain (session, domain, modelName) {
  const modelDomain = session.domainRestriction &&
        session.domainRestriction[modelName]
  return modelDomain ? [domain, modelDomain] : domain
}
...
Record.prototype.read = function (name) {
  ...
  var args = [
    ['id', '=', this.id], null, null, null
  ]
  if (this.session.party) {
    args[0] = alterDomain(this.session, args[0], this.model.name)
  }
  ...
  var promise = this.session.rpc('model.' + this.model.name + '.' + methods.modelSearchRead,
    args, this.getContext())
    .then(
      (results) => {
        var result = results[0]
        assert(result || !this.session.party, 'you can\'t access this record')
...
Record.prototype.write = function () {
  ...
  assert(!this.isWriting(), 'concurrent request')
...
Record.prototype.del = function () {
  dRec('%s(%s): del', this.model.name, this.id)
  if (this.delPromise) {
    return this.delPromise
  }
  assert(!this.session.party, 'you can\'t delete a record')
  assert(!this.isReading(), 'record being read')
  assert(!this.isWriting(), 'record being written')
  assert(this.id > 0, 'del non registered records')
  assert(!this.hasChanged(), 'discarding changes')
  this.delPromise = this.session.rpc('model.' + this.model.name + '.' +
      methods.modelDelete, [[this.id]], this.getContext())
    .then(() => {
      this.delPromise = true
      resetAttrs.call(this)
      this.detach()
      this.id = 0
    }, (err) => {
...
Group.prototype.save = function () {
  dGrp('%s(%s): save %s', this.model.name, this.id)
  assert(this.every((rec) => !rec.isReading()), 'concurrent request')
  assert(this.every((rec) => rec.id > 0), 'save non registered records')
  var args = _.flatten(_.map(_.omit(this.groupBy(rec =>
      !_.isEmpty(rec.changes) && JSON.stringify(rec.changes)
    ), false), records => {
        var changes = _.keys(records[0].changes)
        return [ records.map(record => record.id),
            _.zipObject(changes, records[0].get(changes))]
  }))
  if(!args.length) {return Promise.resolve()}
  var ids = _.flatten(args.filter((_, i) => i%2 === 0))
  var writePromise = this.session.rpc(`model.${this.model.name}.${methods.modelWrite}`,
    args, this.getContext())
    .then(() => {
      this.forEach((rec) => { resetAttrs.call(rec); rec.writePromise = true })
    }, (err) => {
      this.forEach((rec) => { rec.writePromise = false })
      return Promise.reject(err)
    })
  this.filter(rec => ids.includes(rec.id)).forEach((rec) => {
    rec.writePromise = writePromise
  })
  return writePromise
}
...
Group.count = function (session, modelName, domain, context) {
  ...
  if (session.party) {
    domain = alterDomain(session, domain, modelName)
  }
...
Group.search = function (session, modelName, criteria, context) {
  ...
  if (session.party) {
    criteria.domain = alterDomain(session, criteria.domain, modelName)
  }
