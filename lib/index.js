...
function instanciate(record, field, value) {
  if (field.cls === 'record') {
    return Record(record.session, field.desc.relation, value)
      .then((rec) => {
        rec.setContext(record.getContext());
        return rec;
      });
  }
    ...
    return Group(record.session, field.desc.relation, record, field.name)
      .then((group) => {
        group.setContext(record.getContext());
        group.init(value);
        return group;
      });
  }
...
Record.prototype.toJSON = function (loaded) {
  dRec(this.model.name + ': toJSON');
  if (!loaded) {
    loaded = {};
  }
  if (loaded[this.model.name + '-' + this.id]) {
    return loaded[this.model.name + '-' + this.id];
  }
  else {
    loaded[this.model.name + '-' + this.id] = {
      _: this.model.name,
      id: this.id
    };
  }
  var l = _.mapObject(this.attrs, (v, k) => ({
    name: k,
    field: this.model.fields[k],
  ...
  return _.extendOwn({
    _: this.model.name,
    id: this.id
  }, _.object(_.pluck(l, 'name'), _.pluck(l, 'value')));
};
...
Group.prototype.toJSON = function (loaded) {
  dGrp(this.model.name + ': toJSON');
  if (!loaded) {
    loaded = {};
  }
  return this.map((rec) => {
    return rec.toJSON(loaded);
  });
};
