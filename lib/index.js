...
function instFromObj(record, field, value) {
  return Record(record.session, field.desc.relation, value.id)
    .then((inst) => inst.set(_.omit(value, 'id'), {
      sync: false,
      touch: false,
      emit: false
    }));
...
Record.prototype.get = function (name, options) {
  ...
  assert(_.isEmpty(_.omit(options, ['rpc', 'inst', 'parent'])),
    'unknown options');
  _.defaults(options, {
    rpc: false,
    inst: true,
...
Record.prototype.set = function (name, value, options) {
  ...
  var promises = [];
  _.each(l, (e) => {
    if (e.field.cls === 'record' && _.isPlainObject(e.value)) {
      promises.push(instFromObj(this, e.field, e.value)
        .then((inst) => {
          e.value = inst.id;
        }));
    }
    if (e.field.cls === 'group') {
      _.each(e.value, (v, i) => {
        if (_.isPlainObject(v)) {
          promises.push(instFromObj(this, e.field, v)
            .then((inst) => {
              e.value[i] = inst.id;
            }));
        }
...
Record.prototype.onChange = function (names) {
  dRec(this.model.name + ': onChange', names.join(','));
...
Record.prototype.onChangeWith = function (names) {
  dRec(this.model.name + ': onChangeWith', names.join(','));
  var fieldNames = [];
  var postponed = [];
  var paths = [];
...
Group.prototype.read = function (name) {
  ...
  var promise = this.session.rpc('model.' + this.model.name + '.' + methods.modelRead,
      args, this.getContext())
    .then(results => Promise.all(_.map(results, (result, i) => {
      var rec = starting[i];
