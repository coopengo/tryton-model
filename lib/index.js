...
function alterDomain (party, domain, modelName) {
  const modelDomain = config.domain[modelName]
  const alteredDomain = modelDomain ? [ 'AND', domain ] : domain
  modelDomain && modelDomain.forEach(field => {
    alteredDomain.push([field, '=', party])
  })
  return alteredDomain
}
...
Record.prototype.read = function (name) {
  ...
  if (this.session.party) {
    args[0] = alterDomain(this.session.party, this.model.name, args[0])
  }
  ...
  var promise = this.session.rpc('model.' + this.model.name + '.' + methods.modelSearchRead,
    ...
    .then(
      (results) => {
        ...
        assert(result || !this.session.party, 'you can`t acces this token')
...
Group.count = function (session, modelName, domain, context) {
  ...
  if (session.party) {
    domain = alterDomain(session.party, domain, modelName)
  }
...
Group.search = function (session, modelName, criteria, context) {
  ...
  if (session.party) {
    criteria.domain = alterDomain(session.party, criteria.domain, modelName)
  }
