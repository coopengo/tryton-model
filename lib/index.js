...
function updateFlagsAfterRead(name, result) {
  if (!name) {
    this.attrs = {};
    this.changes = {};
    this.readFields = {
...
Record.prototype.detach = function () {
  ...
  this.removeListener('remove', updateCache);
};
  ...
  var promise = this.session.rpc('model.' + this.model.name + '.' + methods.modelRead,
    ...
    .then(
      (results) => {
        ...
        updateFlagsAfterRead.call(this, name, result);
...
Group.prototype.read = function (name) {
  ...
  var promise = this.session.rpc('model.' + this.model.name + '.' + methods.modelRead,
    ...
    .then(results => Promise.all(_.map(results, (result, i) => {
      var rec = starting[i];
      updateFlagsAfterRead.call(rec, name, result);
