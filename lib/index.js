...
Group.prototype.read = function (name) {
  ...
  if (!name) {
    started = this.filter((rec) => _.isPromise(rec.readPromise));
    starting = this.filter((rec) => !_.isPromise(rec.readPromise));
  }
  else {
    started = [];
...
Group.group = function (session, iterable) {
  ...
  _.assert(!_.some(iterable, (rec) => !(rec instanceof Record) || rec.model.name !==
    modelName), 'bad argument');
  var group = new Group(session, model);
  group.records = _.map(iterable, _.identity);
  return group;
};
//
// exports
